...
const initialKitchenOrders: KitchenOrder[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: 'kitem004b',
        menuItemId: 'coldDrink02',
        name: 'Cola',
        quantity: 2,
        status: 'Ready',
        categoryKey: 'coldDrink',
      },
    ],
  },
];
...
export default function KitchenStaffPage() {
  ...
  const [orders, setOrders] = useState<KitchenOrder[]>(
    initialKitchenOrders.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime()),
  );
    ...
    <main className={styles.container}>
      ...
      {activeOrders.length === 0 ? (
        <p className={styles.noOrdersMessage}>
          {t('no_orders_to_prepare_message', 'No orders currently need preparation.')}
        </p>
      ) : (
          ...
          {activeOrders.map((order) => (
            <div key={order.id} className={styles.orderCard}>
              <div className={styles.orderCardHeader}>
                <h3>
                  {t('order_id_label', 'Order ID')}: {order.id}
                </h3>
                <span className={styles.orderInfo}>
                  {t(
                    order.orderType === 'Dine-in' ? 'checkout_order_type_dine_in' : 'checkout_order_type_pickup',
                    order.orderType,
                  )}
                  {order.orderType === 'Dine-in' &&
                    order.tableNumber &&
                    ` (${t('table_number_label_short', 'Table')}: ${order.tableNumber})`}
                </span>
              </div>
              {order.orderNotes && (

...
| `background-position: right …` | Has no logical form, and is a keyword, so `--dir` cannot express it. A gutter that clears a chevron must mirror **with** the chevron, or text runs under it. | Mirror the padding and the background position together, in a `[dir='rtl']` rule. |

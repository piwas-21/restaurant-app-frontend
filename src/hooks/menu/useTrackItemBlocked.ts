...
/**
 * Refusals already reported this page-load, keyed `itemId:orderType`.
 *
 ...
 * second refusal is the signal that the guest has not found one that works.
 *
 ...
 */
export function useTrackItemBlocked(itemId: string | undefined, notice: AvailabilityNotice | null): void {
  const { state } = useOrderType();
  const orderType = state.orderType;

  const blocked = notice?.tone === 'blocked';
  const key = blocked && itemId && orderType !== null ? `${itemId}:${orderType}` : null;

...
| FS4 | **Add file upload validation** - type whitelist + size limit on client side — **product images DONE** (Track F F1c: `utils/imageUploadRules.ts`, 10 MB = the server's own cap); category image / logo uploaders remain | HIGH     | ~~`components/admin/product-details/ImageGallery.tsx`~~ (rewritten to `product-editor/` in #215) and the remaining upload components |

...
**`prettier --check` automated as of Sprint 2**. `eslint --max-warnings=0` and `tsc --noEmit` deferred — there are 200+ pre-existing lint warnings and a handful of test-file type errors that need clearing before those gates can flip blocking. Tracked as Sprint 2.5 in [docs/QUALITY-SECURITY-PLAN.md](docs/QUALITY-SECURITY-PLAN.md). SAST quality gate (SonarCloud) lands in Sprint 3.

...
4. **Injection — SSR inline `<style>`.** `layout.tsx` (a Server Component) fetches the key server-side (`revalidate: 30`, matching the app's existing RestaurantInfo freshness) and emits an inline `<style>` **after** the `@active-template/tokens.css` + `globals.css` imports, re-declaring `:root{…}` + `html[data-theme='dark']{…}` (paired light + dark). At equal specificity, source order wins, so the runtime block overrides the baked template tokens. **Not** client-side `setProperty` (which flashes the default palette before hydration — unacceptable FOUC on a public page). The `<style>` must land in `<head>` on first paint (React 19 hoists `<style>`/`<link>` out of the component tree, or place it explicitly) so the tokens are server-rendered before any component reads them and the server/client markup matches — no hydration mismatch.

...
4. **Shared surfaces (menu, cart, checkout, reservations, auth) are skinned via semantic design tokens only** (DESIGN-SYSTEM.md §2, implemented starting T1). Structural per-surface component swaps ("slots") are explicitly **out of v1**. Staff/admin chrome is not templated in v1.

...
import { MAX_SHIFTS_PER_DAY, asEditableShift, type EditableShift } from './workingHoursDay';
...
interface WorkingHoursDayShiftsProps {
  ...
  dayName: string;
  disabled: boolean;
  onChange: (shifts: EditableShift[]) => void;
}
 ...
 */
export default function WorkingHoursDayShifts({
  shifts,
  dayName,
  disabled,
  onChange,
}: Readonly<WorkingHoursDayShiftsProps>) {
  ...
  const addShift = () => {
    ...
    onChange([...shifts, asEditableShift({ openTime: last?.closeTime ?? '18:00:00', closeTime: '23:00:00' })]);
  };
  ...
  return (
    <div className={styles.shifts}>
      {shifts.map((shift, index) => (
        <div key={shift.uid} className={styles.shiftRow}>
          <input
            type="time"

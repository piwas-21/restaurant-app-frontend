...
Fixed on this branch: `TENANT_LOCALE` (`NEXT_PUBLIC_TENANT_LOCALE`, `de-CH` fallback) now drives
both formatters, and it is plumbed through the Dockerfile and the tenant build workflows as a
`locale` input beside `currency`.
